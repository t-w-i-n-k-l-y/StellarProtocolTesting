...
func PivotFieldKeyTest(key string, value string) {
	fmt.Println()
	fmt.Println("<------------------------------ Pivot field - Key ------------------------------>")

	fmt.Println(" - Pivot Field Key MDO Key Field")
	fmt.Println("name(text): ", key)
	fmt.Println("Pivot Field Key MDO Key actual length: ", len(key))
	if len(key) != 64 {
		fmt.Println("Error: The length of the key field is not 64")
	}

	fmt.Println()
	fmt.Println(" - Pivot Field Key Value Field")
	...
	fmt.Println("Fields in the value field of the Pivot Field Key Manage Data:")
	byteArryToHexString(byteArrayMetricPublisher[0:64], "Pivot Field Key 2nd 64 bytes:")
